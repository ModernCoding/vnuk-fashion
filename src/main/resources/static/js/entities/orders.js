$(function(){

    //  DELETING ORDER
    $('.my-order-to-delete').on('click', function (e){
        e.preventDefault();

        var $order = $(this);
        var orderId = $order.val();

        
        $.ajax({
           
            type: "DELETE",
            url: "/orders/" + orderId,
            
            data: {},
            
            success: function() {
                
                $('tr[data-order-id="' + orderId + '"]').remove();
                
                $('#my-notice').addClass('my-notice-green').removeClass("my-no-line-height");
                $('#my-notice i').addClass('far fa-smile');
                $('#my-notice span:last-child').text("Order #" + orderId + " has successfully been deleted.");
                    
            },
            
            
            error: function() {
            	
                $('#my-notice').addClass('my-notice-red').removeClass("my-no-line-height");
                $('#my-notice i').addClass('fas fa-dizzy');
                $('#my-notice span:last-child').text("Something went wrong with deletion of order #" + orderId);
            
            }
            
        });
        
    });

});
